...
import { FiUser, FiMail, FiEdit2, FiArrowLeft, FiMapPin } from "react-icons/fi";
...
export default function Profile() {
  ...
  const handleSave = () => {
    ...
    axios
      .put(`${API_URL}/profile/`, updatedData, {
        headers: {
    ...
    >
      ...
      <div className="relative z-10 bg-white/15 backdrop-blur-md border border-white/20 rounded-3xl shadow-2xl p-10 max-w-lg w-[90%]">
        ...
        {user ? (
            ...
            {editing ? (
              <div className="space-y-4">
                <div>
                  <label
                    htmlFor="bio"
                    className="block text-sm text-green-100 mb-1"
                  >
                    Bio
                  </label>
                  <input
          ...
          <p className="text-green-200">Loading profile...</p>
        )}
      </div>
    </div>
  );
